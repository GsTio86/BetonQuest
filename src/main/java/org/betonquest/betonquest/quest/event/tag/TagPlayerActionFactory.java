 ...
 */
public class TagPlayerActionFactory implements PlayerActionFactory, StaticActionFactory {
    /**
     ...
     */
    public TagPlayerActionFactory(final BetonQuest betonQuest, final Saver saver) {
    ...
    private TagPlayerAction createAddTagEvent(final String... tags) {
        final TagChanger tagChanger = new AddTagChanger(tags);
        return new TagPlayerAction(betonQuest::getOfflinePlayerData, tagChanger);
    }

    private TagPlayerAction createDeleteTagEvent(final String... tags) {
        final TagChanger tagChanger = new DeleteTagChanger(tags);
        return new TagPlayerAction(betonQuest::getOfflinePlayerData, tagChanger);
    }

    private StaticAction createStaticDeleteTagEvent(final String... tags) {
        final TagPlayerAction deleteTagEvent = createDeleteTagEvent(tags);
        final List<StaticAction> staticActions = new ArrayList<>(tags.length + 1);
        staticActions.add(new OnlineProfileGroupStaticEventAdapter(PlayerConverter::getOnlineProfiles, deleteTagEvent));
        for (final String tag : tags) {
            staticActions.add(new DatabaseSaverStaticEvent(saver, () -> new Saver.Record(UpdateType.REMOVE_ALL_TAGS, tag)));
        }
        return new SequentialStaticEvent(staticActions.toArray(new StaticAction[0]));
    }
}
