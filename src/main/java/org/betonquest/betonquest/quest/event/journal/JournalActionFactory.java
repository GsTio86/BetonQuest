 ...
 */
public class JournalActionFactory implements PlayerActionFactory, StaticActionFactory {
    /**
     ...
     */
    public JournalActionFactory(final BetonQuestLoggerFactory loggerFactory, final BetonQuest betonQuest, final InstantSource instantSource, final Saver saver) {
    ...
    private JournalPlayerAction createJournalDeleteEvent(final Instruction instruction) throws InstructionParseException {
        ...
        return new JournalPlayerAction(betonQuest, journalChanger, notificationSender);
    }

    private JournalPlayerAction createJournalAddEvent(final Instruction instruction) throws InstructionParseException {
        final String entryName = Utils.addPackage(instruction.getPackage(), instruction.getPart(2));
        final JournalChanger journalChanger = new AddEntryJournalChanger(instantSource, entryName);
        final NotificationSender notificationSender = new IngameNotificationSender(loggerFactory.create(JournalPlayerAction.class), instruction.getPackage(), instruction.getID().getFullID(), NotificationLevel.INFO, "new_journal_entry");
        return new JournalPlayerAction(betonQuest, journalChanger, notificationSender);
    }

    private JournalPlayerAction createJournalUpdateEvent() {
        final JournalChanger journalChanger = new NoActionJournalChanger();
        final NotificationSender notificationSender = new NoNotificationSender();
        return new JournalPlayerAction(betonQuest, journalChanger, notificationSender);
    }

    private StaticAction createStaticJournalDeleteEvent(final Instruction instruction) throws InstructionParseException {
        final JournalPlayerAction journalDeleteEvent = createJournalDeleteEvent(instruction.copy());
        final String entryName = Utils.addPackage(instruction.getPackage(), instruction.getPart(2));
